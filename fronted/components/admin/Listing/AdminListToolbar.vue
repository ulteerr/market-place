<template>
  <div class="grid gap-3 lg:grid-cols-[1fr_auto_auto_auto]">
    ...
    <button type="button" class="admin-button-secondary rounded-lg px-3 py-2 text-xs" :disabled="loading" @click="$emit('apply')">
      Найти
    </button>

    <button type="button" class="admin-button-secondary rounded-lg px-3 py-2 text-xs" :disabled="loading" @click="$emit('reset')">
      Сбросить
    </button>
  </div>
</template>
