<template>
  <section class="profile-page mx-auto w-full max-w-3xl space-y-6">
    <div class="profile-card rounded-2xl p-6 lg:p-8">
      <h2 class="text-2xl font-semibold">Мой профиль</h2>
      <p class="profile-muted mt-2 text-sm">Редактирование личных данных текущего пользователя.</p>
    </div>
    ...
    <article class="profile-card rounded-2xl p-5 lg:p-6">
      <form class="space-y-3" @submit.prevent="submitForm">
        ...
        <div class="flex gap-2">
          ...
          <NuxtLink to="/admin" class="admin-button-secondary rounded-lg px-4 py-2 text-sm">Отмена</NuxtLink>
        </div>
      </form>
    </article>
  </section>
...
const submitForm = async () => {
  ...
  try {
    ...
    formError.value = getApiErrorMessage(error, 'Не удалось обновить профиль.')
