<template>
  <AdminEntityIndex
    ...
    @update:search-value="(value) => (listState.searchInput.value = String(value || ''))"
...
import type {
  AdminActionLogItem,
  AdminActionLogsListParams,
} from '~/composables/useAdminActionLogs';
...
type ActionLogSortField = 'created_at' | 'event' | 'model_type' | 'user_id';
const allowedSortFields: ActionLogSortField[] = ['created_at', 'event', 'model_type', 'user_id'];
const eventFilterOptions = ['all', 'create', 'update', 'delete'] as const;
...
const allowedEventFilters = new Set<string>(eventFilterOptions);
const allowedModelFilters = new Set<string>(modelFilterOptions);
...
const readCustomStateFromQuery = () => {
  const rawEvent = String(route.query.event ?? 'all');
  eventFilter.value = allowedEventFilters.has(rawEvent) ? rawEvent : 'all';

  const rawModel = String(route.query.model ?? 'all');
  modelFilter.value = allowedModelFilters.has(rawModel) ? rawModel : 'all';

  userFilter.value = String(route.query.user ?? '').trim();
...
const buildCustomListQuery = (): Partial<AdminActionLogsListParams> => ({
...
const buildCustomQuery = (): Record<string, string | undefined> => ({
  event: eventFilter.value === 'all' ? undefined : eventFilter.value,
  model: modelFilter.value === 'all' ? undefined : modelFilter.value,
  user: userFilter.value.trim() || undefined,
  date_from: dateRangeFilter.value[0] || undefined,
...
const {
  ...
  onUpdatePerPage,
  onResetFilters,
} = useAdminCrudIndex<AdminActionLogItem, AdminActionLogsListParams>({
  settingsKey: 'action_logs',
  defaultSortBy: 'created_at',
  defaultSortDir: 'desc',
  allowedSortBy: [...allowedSortFields],
  defaultPerPage: 20,
  perPageOptions,
  listErrorMessage: t('admin.actionLogs.errors.load'),
  deleteErrorMessage: t('admin.actionLogs.errors.load'),
  list: actionLogsApi.list,
...
const getSortIndicator = (field: ActionLogSortField): string => {
  if (listState.sortBy.value !== field) {
    return '';
  }
  return listState.sortDir.value === 'asc' ? '↑' : '↓';
};

const onSort = async (field: string) => {
  if (!allowedSortFields.includes(field as ActionLogSortField)) {
    return;
  }

  await applyAndFetch(listState.toggleSort(field));
};
...
const onEventChange = async (value: string | number | (string | number)[]) => {
  ...
  const normalizedValue = String(next || 'all');
  eventFilter.value = allowedEventFilters.has(normalizedValue) ? normalizedValue : 'all';
  await applyAndFetch(1);
};

const onModelChange = async (value: string | number | (string | number)[]) => {
  const next = Array.isArray(value) ? value[0] : value;
  const normalizedValue = String(next || 'all');
  modelFilter.value = allowedModelFilters.has(normalizedValue) ? normalizedValue : 'all';
  await applyAndFetch(1);
};
...
const onPageChange = async (nextPage: number) => {
  await applyAndFetch(nextPage);
};

useDebouncedSearch(
  [() => listState.searchInput.value, () => userFilter.value],
  async () => {
