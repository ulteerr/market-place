<template>
  <AdminEntityIndex
    page-class="geo-regions-page"
    max-width-class="max-w-7xl"
    :title="t('admin.geo.regions.index.title')"
    :subtitle="t('admin.geo.regions.index.subtitle')"
    create-to="/admin/geo/regions/new"
    :show-create="true"
    :create-label="t('admin.geo.regions.index.createLabel')"
    :search-value="listState.searchInput.value"
    :search-placeholder="t('admin.geo.regions.index.searchPlaceholder')"
    :show-apply="false"
            ...
            <tr>
              <th>
                <button type="button" class="sort-btn" @click="onToggleSort('name')">
                  {{ t('admin.geo.regions.index.headers.name') }}
                  {{ listState.sortMark('name') }}
                </button>
              </th>
              <th>{{ t('admin.geo.regions.index.headers.countryId') }}</th>
              <th class="text-right">{{ t('admin.geo.regions.index.headers.actions') }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="loading">
            ...
            <tr v-else-if="!items.length">
              <td colspan="3" class="admin-muted py-5 text-center text-sm">
                {{ t('admin.geo.regions.index.empty') }}
              </td>
            </tr>
            <tr v-for="item in items" :key="item.id">
              ...
              <td>
                <AdminCrudActions
                  ...
                  :edit-to="`/admin/geo/regions/${item.id}/edit`"
                  :can-show="true"
                  :can-edit="true"
                  :can-delete="true"
...
const {
  ...
  confirmRemoveItem,
  cancelRemoveItem,
} = useAdminCrudIndex<AdminGeoRegion>({
  settingsKey: 'geo-regions',
  defaultSortBy: 'name',
  defaultPerPage: 20,
  listErrorMessage: t('admin.geo.regions.errors.loadList'),
  deleteErrorMessage: t('admin.geo.regions.errors.delete'),
  list: api.list,
  remove: api.remove,
...
const cardSortFields = computed(() => [
  { value: 'name', label: t('admin.geo.regions.index.sort.name') },
  { value: 'country_id', label: t('admin.geo.regions.index.sort.countryId') },
]);
...
const onRemove = (item: AdminGeoRegion) => {
  removeItem(item, {
    ...
    confirmMessage: t('admin.geo.regions.confirmDelete', { name: item.name }),
