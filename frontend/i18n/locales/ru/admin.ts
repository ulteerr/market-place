export default {
  layout: {
    panelTitle: 'Admin Panel',
    shortPanelTitle: 'AP',
    closeSidebar: 'Закрыть меню',
    openSidebar: 'Открыть меню',
    sidebarToggleExpand: 'Expand menu',
    sidebarToggleCollapse: 'Collapse menu',
    heading: 'Административная панель',
    breadcrumbsAria: 'Хлебные крошки',
    toggleLightMode: 'Toggle light mode',
    toggleDarkMode: 'Toggle dark mode',
    menu: {
      dashboard: 'Главная',
      settings: 'Настройки',
      users: 'Пользователи',
      roles: 'Роли',
      categories: 'Категории',
      activities: 'Активности',
      activityLeads: 'Заявки активности',
      organizations: 'Организации',
      children: 'Дети',
      actionLogs: 'Журнал изменений',
      monitoring: 'Мониторинг',
      metroLines: 'Ветки',
      metroStations: 'Станции',
      geoCountries: 'Страны',
      geoRegions: 'Регионы',
      geoCities: 'Города',
      geoDistricts: 'Районы',
    },
    sections: {
      system: 'Система',
      geo: 'География',
      metro: 'Метро',
      organization: 'Организация',
    },
    user: {
      guest: 'Гость',
      noEmail: 'Нет email',
    },
  },
  userMenu: {
    profile: 'Профиль',
    settings: 'Настройки',
    logout: 'Выйти',
  },
  entity: {
    shownCount: 'Показано {shown} из {total}.',
    desktopTable: 'Desktop: таблица',
    desktopCards: 'Desktop: карточки',
    modePlaceholder: 'Таблица',
    modes: {
      table: 'Таблица',
      tableCards: 'Таблица + карточки',
      cards: 'Карточки',
    },
  },
  toolbar: {
    search: 'Поиск',
    perPage: 'По {count}',
    find: 'Найти',
    reset: 'Сбросить',
  },
  pagination: {
    summary: 'Страница {current} / {last}. На странице: {perPage}.',
    back: 'Назад',
    forward: 'Вперед',
  },
  actions: {
    show: 'Показать',
    edit: 'Редактировать',
    delete: 'Удалить',
    rollback: 'Откатить',
  },
  files: {
    avatarHint: 'PNG/JPG/WEBP, до 5MB',
  },
  permissions: {
    title: 'Права роли',
    roleHint: 'Отметьте права, которые роль получает по умолчанию.',
    userTitle: 'Персональные права пользователя',
    userHint: 'Allow выдает право точечно, Deny запрещает право точечно (приоритетнее ролей).',
    allow: 'Разрешить',
    deny: 'Запретить',
    scopes: {
      admin: 'Админка',
      org: 'Организация',
      user: 'Пользователь',
    },
    codes: {
      admin: {
        panel: {
          access: 'Доступ в админ-панель',
        },
        users: {
          read: 'Просмотр пользователей в админке',
          create: 'Создание пользователей в админке',
          update: 'Изменение пользователей в админке',
          delete: 'Удаление пользователей в админке',
        },
        roles: {
          read: 'Просмотр ролей в админке',
          create: 'Создание ролей в админке',
          update: 'Изменение ролей в админке',
          delete: 'Удаление ролей в админке',
        },
        changelog: {
          read: 'Просмотр changelog в админке',
          rollback: 'Запрос rollback changelog в админке',
        },
        action_log: {
          read: 'Просмотр action log в админке',
          update: 'Изменение action log в админке',
          delete: 'Удаление action log в админке',
        },
        monitoring: {
          read: 'Просмотр мониторинга в админке',
        },
        activity_leads: {
          read: 'Просмотр заявок activities в админке',
          update: 'Изменение статусов заявок activities в админке',
        },
      },
      org: {
        company: {
          profile: {
            read: 'Просмотр личного кабинета компании',
            update: 'Редактирование личного кабинета компании',
            delete: 'Удаление личного кабинета компании',
          },
        },
        members: {
          read: 'Просмотр участников организации',
          write: 'Управление участниками организации',
        },
        children: {
          read: 'Просмотр детей в организации',
          write: 'Управление детьми в организации',
        },
        activity_leads: {
          read: 'Просмотр заявок activities в кабинете организации',
          update: 'Изменение статусов заявок activities в кабинете организации',
        },
      },
      user: {
        profile: {
          read: 'Просмотр личного кабинета пользователя',
          update: 'Редактирование личного кабинета пользователя',
          delete: 'Удаление личного кабинета пользователя',
        },
      },
    },
  },
  changelog: {
    title: 'История изменений',
    subtitle: 'Лог правок по текущей сущности.',
    version: 'Версия',
    actor: 'Пользователь',
    actorMe: 'Я',
    changedFields: 'Изменённые поля',
    was: 'было',
    now: 'стало',
    set: 'установлено',
    complexChanged: 'изменено',
    showJson: 'Показать JSON',
    hideJson: 'Скрыть JSON',
    createdData: 'Создано',
    mediaSnapshots: 'Медиа-снапшоты',
    before: 'До',
    after: 'После',
    preview: {
      title: 'Изображение',
      open: 'Открыть изображение',
    },
    empty: 'Пока нет изменений.',
    rollback: {
      title: 'Откат изменений',
      confirm: 'Откатить к версии #{version} ({event})?',
      fromVersion: 'Откат из версии #{version}',
    },
    fields: {
      common: {
        settings: 'Настройки',
        locale: 'Язык',
        theme: 'Тема',
        collapse_menu: 'Свернуть меню',
      },
      profile: {
        first_name: 'Имя',
        last_name: 'Фамилия',
        middle_name: 'Отчество',
        gender: 'Пол',
        email: 'Email',
        phone: 'Телефон',
        birth_date: 'Дата рождения',
      },
      user: {
        first_name: 'Имя',
        last_name: 'Фамилия',
        middle_name: 'Отчество',
        gender: 'Пол',
        email: 'Email',
        phone: 'Телефон',
        birth_date: 'Дата рождения',
        roles: 'Роли',
        avatar_id: 'Аватар',
        permission_overrides: 'Персональные права',
      },
      role: {
        code: 'Code',
        label: 'Label',
        is_system: 'Системная роль',
      },
      child: {
        first_name: 'Имя',
        last_name: 'Фамилия',
        middle_name: 'Отчество',
        gender: 'Пол',
        birth_date: 'Дата рождения',
        user_id: 'Пользователь',
      },
      organization: {
        name: 'Название',
        description: 'Описание',
        address: 'Адрес',
        phone: 'Телефон',
        email: 'Email',
        status: 'Статус',
        source_type: 'Источник',
        ownership_status: 'Статус владения',
        owner_user_id: 'Владелец',
      },
    },
    events: {
      create: 'Создание',
      update: 'Обновление',
      delete: 'Удаление',
      restore: 'Восстановление',
    },
    errors: {
      load: 'Не удалось загрузить историю изменений.',
      rollback: 'Не удалось выполнить откат.',
    },
  },
  dashboard: {
    title: 'Панель управления',
    subtitle: 'Выберите модуль в левом меню. Здесь отображается контент выбранного раздела.',
    usersTitle: 'Пользователи',
    usersSubtitle: 'Управление ролями, доступами и профилями администраторов.',
    contentTitle: 'Контент',
    contentSubtitle: 'Редактирование разделов, настройка видимости и модерирование.',
    stats: {
      totalUsers: 'Всего пользователей',
      onlineUsers: 'Сейчас онлайн',
      onlineHint: 'Обновляется в реальном времени',
      loadError: 'Не удалось загрузить статистику пользователей.',
    },
  },
  categories: {
    fields: {
      name: 'Название',
      slug: 'Slug',
      parent: 'Родительская категория',
      sortOrder: 'Порядок сортировки',
      isActive: 'Активная категория',
    },
    index: {
      title: 'Категории',
      subtitle: 'Управление деревом категорий и их статусами.',
      createLabel: 'Новая категория',
      searchPlaceholder: 'Найти категорию по названию или slug',
      empty: 'Категории пока не добавлены.',
      root: 'Корневая категория',
      headers: {
        name: 'Название',
        slug: 'Slug',
        parent: 'Родитель',
        sortOrder: 'Порядок',
        status: 'Статус',
        children: 'Children',
        activities: 'Activities',
        actions: 'Действия',
      },
      card: {
        slug: 'Slug: {value}',
        parent: 'Родитель: {value}',
        sortOrder: 'Порядок: {value}',
        status: 'Статус: {value}',
        children: 'Children: {value}',
        activities: 'Activities: {value}',
      },
      filteredByParent: 'Показаны дочерние категории для: {value}',
      clearParentFilter: 'Показать все категории',
      sort: {
        name: 'Названию',
        slug: 'Slug',
        sortOrder: 'Порядку',
        status: 'Статусу',
      },
    },
    status: {
      active: 'Активна',
      inactive: 'Неактивна',
    },
    new: {
      title: 'Новая категория',
      subtitle: 'Создание корневой категории или подкатегории.',
      parentPlaceholder: 'Без родителя',
      isActiveHint: 'Неактивные категории скрываются из public navigation.',
      slugAutofill: 'Авто',
      slugHint: 'Slug можно отредактировать вручную, если автогенерация не подходит.',
      slugLoading: 'Подбираем slug...',
      saving: 'Создание...',
      errors: {
        create: 'Не удалось создать категорию.',
        loadParents: 'Не удалось загрузить список родительских категорий.',
        previewSlug: 'Не удалось сгенерировать slug.',
      },
    },
    show: {
      title: 'Категория',
      subtitle: 'Просмотр данных категории и ее дочерних узлов.',
      labels: {
        children: 'Дочерние категории',
        activities: 'Связанные activities',
      },
      childrenTitle: 'Дочерние категории',
      openChildren: 'Открыть дочерние',
      errors: {
        invalidId: 'Некорректный идентификатор категории.',
        load: 'Не удалось загрузить категорию.',
      },
    },
    edit: {
      title: 'Редактирование категории',
      subtitle: 'Обновление названия, slug и положения категории в дереве.',
      parentPlaceholder: 'Без родителя',
      isActiveHint: 'Неактивные категории скрываются из public navigation.',
      slugAutofill: 'Авто',
      slugHint: 'Slug можно отредактировать вручную, если автогенерация не подходит.',
      slugLoading: 'Подбираем slug...',
      saving: 'Сохранение...',
      errors: {
        invalidId: 'Некорректный идентификатор категории.',
        load: 'Не удалось загрузить категорию.',
        update: 'Не удалось сохранить категорию.',
        loadParents: 'Не удалось загрузить список родительских категорий.',
        previewSlug: 'Не удалось сгенерировать slug.',
      },
    },
    confirmDelete: 'Удалить категорию «{name}»?',
    errors: {
      loadList: 'Не удалось загрузить список категорий.',
      delete: 'Не удалось удалить категорию.',
    },
  },
  activities: {
    fields: {
      organization: 'Организация',
      location: 'Локация',
      category: 'Категория',
      rootCategory: 'Основная категория',
      subcategory: 'Подкатегория',
      name: 'Название',
      slug: 'Slug',
      shortDescription: 'Короткое описание',
      description: 'Описание',
      minAge: 'Мин. возраст',
      maxAge: 'Макс. возраст',
      capacity: 'Вместимость',
      priceFrom: 'Цена от',
      priceTo: 'Цена до',
      currency: 'Валюта',
      status: 'Статус',
      publishedAt: 'Дата публикации',
      isFeatured: 'Показывать в витрине',
    },
    index: {
      title: 'Активности',
      subtitle: 'Стартовый раздел для управления секциями, кружками и другими активностями.',
      createLabel: 'Новая активность',
      searchPlaceholder: 'Найти активность по названию, slug или организации',
      empty: 'Активности пока не добавлены.',
      headers: {
        name: 'Название',
        organization: 'Организация',
        location: 'Локация',
        category: 'Категория',
        status: 'Статус',
        featured: 'Витрина',
        publishedAt: 'Публикация',
        actions: 'Действия',
      },
      card: {
        organization: 'Организация: {value}',
        location: 'Локация: {value}',
        category: 'Категория: {value}',
        status: 'Статус: {value}',
        featured: 'Витрина: {value}',
        publishedAt: 'Публикация: {value}',
      },
      sort: {
        createdAt: 'Созданию',
        name: 'Названию',
        status: 'Статусу',
        publishedAt: 'Публикации',
        featured: 'Витрине',
      },
    },
    new: {
      title: 'Новая активность',
      subtitle: 'Создание новой секции, кружка или другой активности.',
      organizationPlaceholder: 'Выберите организацию',
      locationPlaceholder: 'Сначала выберите организацию',
      rootCategoryPlaceholder: 'Выберите root-категорию',
      categoryPlaceholder: 'Выберите leaf-подкатегорию',
      locationLabel: 'Локация',
      statusPlaceholder: 'Выберите статус',
      isFeaturedHint: 'Витринные активности могут попадать в featured-блоки.',
      saving: 'Создание...',
      errors: {
        create: 'Не удалось создать активность.',
        loadOrganizations: 'Не удалось загрузить организации.',
        loadCategories: 'Не удалось загрузить категории.',
        rootCategoryRequired: 'Выберите основную категорию.',
        subcategoryRequired: 'Выберите подкатегорию.',
      },
    },
    show: {
      title: 'Активность',
      subtitle: 'Просмотр карточки активности и ее связей.',
      schedulesTitle: 'Расписание',
      scheduleTime: '{start} - {end}',
      coverTitle: 'Обложка',
      galleryTitle: 'Галерея',
      errors: {
        invalidId: 'Некорректный идентификатор активности.',
        load: 'Не удалось загрузить активность.',
      },
    },
    edit: {
      title: 'Редактирование активности',
      subtitle: 'Обновление карточки активности и основных параметров.',
      slugAutofill: 'Авто',
      slugHint: 'Slug можно изменить вручную, автогенерация не будет его перезаписывать.',
      slugLoading: 'Генерируем slug...',
      saving: 'Сохраняем...',
      errors: {
        invalidId: 'Некорректный идентификатор активности.',
        load: 'Не удалось загрузить активность.',
        update: 'Не удалось обновить активность.',
        previewSlug: 'Не удалось сгенерировать slug.',
        rootCategoryRequired: 'Выберите основную категорию.',
        subcategoryRequired: 'Выберите подкатегорию.',
      },
    },
    filters: {
      status: 'Статус',
      featured: 'Витрина',
      allStatuses: 'Все статусы',
      allFeatured: 'Все активности',
      featuredOnly: 'Только витринные',
      notFeatured: 'Не витринные',
    },
    status: {
      draft: 'Черновик',
      pendingReview: 'На проверке',
      published: 'Опубликована',
      archived: 'Архив',
    },
    schedule: {
      title: 'Расписание',
      subtitle: 'Добавьте слоты по дням недели и времени проведения.',
      add: 'Добавить слот',
      empty: 'Расписание пока не добавлено.',
      weekday: 'День недели',
      weekdayPlaceholder: 'Выберите день',
      startTime: 'Начало',
      endTime: 'Окончание',
      weekdays: {
        monday: 'Понедельник',
        tuesday: 'Вторник',
        wednesday: 'Среда',
        thursday: 'Четверг',
        friday: 'Пятница',
        saturday: 'Суббота',
        sunday: 'Воскресенье',
      },
    },
    media: {
      title: 'Фотографии',
      subtitle: 'Загрузите обложку и галерею активности.',
      imageHint: 'Поддерживаются PNG, JPEG и WEBP до 10MB.',
      coverTitle: 'Обложка',
      coverHint: 'Главное изображение карточки активности.',
      coverEmpty: 'Обложка пока не добавлена.',
      coverUploadTitle: 'Загрузить обложку',
      coverUploadButton: 'Выбрать обложку',
      coverExistingCaption: 'Текущая обложка',
      coverDraftCaption: 'Новая обложка',
      galleryTitle: 'Галерея',
      galleryHint: 'Дополнительные фотографии активности.',
      galleryEmpty: 'Галерея пока не добавлена.',
      galleryUploadTitle: 'Загрузить фотографии',
      galleryUploadButton: 'Выбрать фотографии',
      galleryExistingCaption: 'Текущее фото',
      galleryDraftCaption: 'Новое фото',
    },
    labels: {
      featuredYes: 'Да',
      featuredNo: 'Нет',
    },
    confirmDelete: 'Удалить активность «{name}»?',
    errors: {
      loadList: 'Не удалось загрузить список активностей.',
      delete: 'Не удалось удалить активность.',
    },
  },
  activityLeads: {
    index: {
      title: 'Заявки активности',
      subtitle: 'Глобальная очередь обращений по секциям, кружкам и другим активностям.',
      searchPlaceholder: 'Поиск: активность, субъект, сообщение, email, телефон',
      empty: 'Заявок пока нет.',
      headers: {
        activity: 'Активность',
        subject: 'Субъект',
        status: 'Статус',
        requestType: 'Тип заявки',
        channels: 'Каналы связи',
        message: 'Сообщение',
        createdAt: 'Создано',
        actions: 'Действия',
      },
      card: {
        status: 'Статус: {value}',
        requestType: 'Тип заявки: {value}',
        channels: 'Каналы: {value}',
        createdAt: 'Создано: {value}',
      },
      sort: {
        createdAt: 'Созданию',
        status: 'Статусу',
      },
      actions: {
        saveStatus: 'Сохранить статус',
      },
    },
    filters: {
      status: 'Статус',
      requestType: 'Тип заявки',
      allStatuses: 'Все статусы',
      allRequestTypes: 'Все типы',
    },
    status: {
      new: 'Новая',
      inProgress: 'В работе',
      contacted: 'Связались',
      registered: 'Записана',
      cancelled: 'Отменена',
    },
    requestType: {
      self: 'На себя',
      child: 'На ребенка',
    },
    errors: {
      loadList: 'Не удалось загрузить заявки активности.',
      updateStatus: 'Не удалось обновить статус заявки.',
    },
  },
  monitoring: {
    title: 'Мониторинг',
    subtitle: 'Сводка событий и инцидентов по доменам observability.',
    filters: {
      domain: 'Домен',
      allDomains: 'Все домены',
      incidentsLimit: 'Лимит инцидентов',
      refresh: 'Обновить',
    },
    cards: {
      eventsTotal: 'События',
      errorsTotal: 'Ошибки',
      avgDuration: 'Средняя длительность (ms)',
      lastEventAt: 'Последнее событие',
    },
    kpi: {
      ok: 'OK',
      total: 'Итого',
      chartTitle: 'События по доменам',
      chartYLabel: 'Значение',
      seriesEvents: 'События',
      seriesErrors: 'Ошибки',
      seriesAvgDuration: 'Средняя длительность',
      errorRate: 'Доля ошибок: {value}%',
      domainsCount: 'Доменов: {count}',
      lastEvent: 'Последнее событие: {value}',
      msValue: '{value} ms',
    },
    incidents: {
      title: 'Последние инциденты',
      empty: 'Инциденты не найдены.',
      headers: {
        time: 'Время',
        domain: 'Домен',
        event: 'Событие',
        status: 'Статус',
        severity: 'Severity',
        component: 'Компонент',
      },
    },
    alerts: {
      title: 'Алерты',
      empty: 'Активных алертов нет.',
    },
    errors: {
      load: 'Не удалось загрузить данные мониторинга.',
    },
  },
  profile: {
    title: 'Мой профиль',
    subtitle: 'Редактирование личных данных текущего пользователя.',
    avatar: {
      title: 'Аватар',
      upload: 'Загрузить',
      delete: 'Удалить аватар',
      previewAlt: 'Аватар пользователя',
      errors: {
        upload: 'Не удалось загрузить аватар.',
        delete: 'Не удалось удалить аватар.',
      },
    },
    fields: {
      firstName: 'Имя',
      lastName: 'Фамилия',
      middleName: 'Отчество',
      gender: 'Пол',
      phone: 'Телефон',
      birthDate: 'Дата рождения',
      email: 'Email',
    },
    genderPlaceholder: 'Выберите пол',
    saving: 'Сохраняем...',
    errors: {
      update: 'Не удалось обновить профиль.',
    },
  },
  settings: {
    title: 'Настройки пользователя',
    subtitle: 'Настройки автоматически сохраняются в браузере и синхронизируются с backend.',
    theme: {
      label: 'Тёмная тема',
      description: 'Переключает цветовую схему интерфейса',
      hint: 'Сохраняется в профиле пользователя',
    },
    menu: {
      label: 'Collapse menu',
      description: 'Сворачивает боковое меню до режима с иконками',
      hint: 'Управляет шириной sidebar в админке',
    },
  },
  genders: {
    male: 'Мужской',
    female: 'Женский',
  },
  users: {
    index: {
      title: 'Пользователи',
      subtitle: 'Просмотр, поиск и редактирование всех пользователей системы.',
      createLabel: 'Новый пользователь',
      searchPlaceholder: 'Поиск: ID, фамилия, имя, отчество, email, телефон, роль',
      empty: 'Пользователи не найдены.',
      headers: {
        id: 'ID',
        thumbnail: 'Аватар',
        lastName: 'Фамилия',
        firstName: 'Имя',
        middleName: 'Отчество',
        gender: 'Пол',
        access: 'Доступ',
        status: 'Статус',
        actions: 'Действия',
      },
      preview: {
        title: 'Изображение',
        open: 'Открыть изображение',
      },
      card: {
        id: 'ID: {value}',
        lastName: 'Фамилия: {value}',
        firstName: 'Имя: {value}',
        middleName: 'Отчество: {value}',
        gender: 'Пол: {value}',
        status: 'Статус: {value}',
      },
      sort: {
        id: 'ID',
        lastName: 'Фамилия',
        firstName: 'Имя',
        middleName: 'Отчество',
        gender: 'Пол',
        access: 'Доступ',
      },
      access: {
        unknown: 'Неизвестно',
        admin: 'Админ-панель',
        basic: 'Без админ-доступа',
      },
      tags: {
        admin: 'Админы',
        basic: 'Обычные пользователи',
      },
    },
    new: {
      title: 'Новый пользователь',
      subtitle: 'Создание пользователя в `/api/admin/users`.',
      fields: {
        firstName: 'Имя',
        lastName: 'Фамилия',
        middleName: 'Отчество',
        gender: 'Пол',
        birthDate: 'Дата рождения',
        email: 'Email',
        phone: 'Телефон',
        password: 'Пароль',
        passwordConfirmation: 'Подтверждение пароля',
        roles: 'Роли',
        avatar: 'Аватар',
      },
      genderPlaceholder: 'Выберите пол',
      rolesPlaceholder: 'Выберите роли',
      saving: 'Сохраняем...',
      errors: {
        create: 'Не удалось создать пользователя.',
      },
    },
    show: {
      title: 'Профиль пользователя',
      subtitle: 'Show-страница пользователя.',
      labels: {
        firstName: 'Имя',
        lastName: 'Фамилия',
        middleName: 'Отчество',
        gender: 'Пол',
        email: 'Email',
        phone: 'Телефон',
        birthDate: 'Дата рождения',
        roles: 'Роли',
        status: 'Статус',
      },
      errors: {
        invalidId: 'Некорректный идентификатор пользователя.',
        load: 'Не удалось загрузить пользователя.',
      },
    },
    edit: {
      title: 'Редактирование пользователя',
      subtitle: 'Edit-страница пользователя.',
      fields: {
        firstName: 'Имя',
        lastName: 'Фамилия',
        middleName: 'Отчество',
        gender: 'Пол',
        birthDate: 'Дата рождения',
        email: 'Email',
        phone: 'Телефон',
        newPassword: 'Новый пароль',
        newPasswordConfirmation: 'Подтверждение нового пароля',
        roles: 'Роли',
        avatar: 'Аватар',
      },
      genderPlaceholder: 'Выберите пол',
      rolesPlaceholder: 'Выберите роли',
      saving: 'Сохраняем...',
      errors: {
        invalidId: 'Некорректный идентификатор пользователя.',
        load: 'Не удалось загрузить пользователя.',
        cannotEditSuperAdmin:
          'Только super_admin может редактировать пользователя с ролью super_admin.',
        cannotEditHigherRole: 'Нельзя редактировать пользователя с ролью выше вашей.',
        update: 'Не удалось обновить пользователя.',
      },
    },
    confirmDelete: 'Удалить пользователя {name}?',
    presence: {
      online: 'Онлайн',
      lastSeen: 'Был в сети {value}',
    },
  },
  children: {
    index: {
      title: 'Дети',
      subtitle: 'CRUD детей в админке.',
      createLabel: 'Новый ребенок',
      searchPlaceholder: 'Поиск: ФИО ребенка или ID пользователя',
      empty: 'Дети не найдены.',
      headers: {
        fullName: 'ФИО',
        birthDate: 'Дата рождения',
        gender: 'Пол',
        user: 'Родитель',
        userId: 'ID пользователя',
        actions: 'Действия',
      },
      card: {
        birthDate: 'Дата рождения: {value}',
        gender: 'Пол: {value}',
        user: 'Родитель: {value}',
        userId: 'ID пользователя: {value}',
      },
      sort: {
        firstName: 'Имя',
        lastName: 'Фамилия',
        middleName: 'Отчество',
        gender: 'Пол',
        birthDate: 'Дата рождения',
        user: 'Родитель',
        userId: 'ID пользователя',
      },
    },
    new: {
      title: 'Новый ребенок',
      subtitle: 'Создание записи ребенка в `/api/admin/children`.',
      fields: {
        firstName: 'Имя',
        lastName: 'Фамилия',
        middleName: 'Отчество',
        gender: 'Пол',
        birthDate: 'Дата рождения',
        userId: 'ID пользователя',
      },
      genderPlaceholder: 'Выберите пол',
      userPlaceholder: 'Поиск пользователя: ФИО или ID',
      saving: 'Сохраняем...',
      errors: {
        create: 'Не удалось создать ребенка.',
      },
    },
    show: {
      title: 'Профиль ребенка',
      subtitle: 'Show-страница ребенка.',
      labels: {
        fullName: 'ФИО',
        birthDate: 'Дата рождения',
        gender: 'Пол',
        user: 'Пользователь',
        userId: 'ID пользователя',
      },
      errors: {
        invalidId: 'Некорректный идентификатор ребенка.',
        load: 'Не удалось загрузить ребенка.',
      },
    },
    edit: {
      title: 'Редактирование ребенка',
      subtitle: 'Edit-страница ребенка.',
      fields: {
        firstName: 'Имя',
        lastName: 'Фамилия',
        middleName: 'Отчество',
        gender: 'Пол',
        birthDate: 'Дата рождения',
        userId: 'ID пользователя',
      },
      genderPlaceholder: 'Выберите пол',
      userPlaceholder: 'Поиск пользователя: ФИО или ID',
      saving: 'Сохраняем...',
      errors: {
        invalidId: 'Некорректный идентификатор ребенка.',
        load: 'Не удалось загрузить ребенка.',
        update: 'Не удалось обновить ребенка.',
      },
    },
    confirmDelete: 'Удалить запись ребенка {name}?',
    errors: {
      loadList: 'Не удалось загрузить список детей.',
      delete: 'Не удалось удалить ребенка.',
    },
  },
  organizations: {
    index: {
      title: 'Организации',
      subtitle: 'CRUD организаций в админке.',
      createLabel: 'Новая организация',
      searchPlaceholder: 'Поиск: название, email, телефон, ID',
      empty: 'Организации не найдены.',
      headers: {
        name: 'Название',
        status: 'Статус',
        ownership: 'Владение',
        owner: 'Владелец',
        createdAt: 'Создано',
        actions: 'Действия',
      },
      card: {
        status: 'Статус: {value}',
        ownership: 'Владение: {value}',
        owner: 'Владелец: {value}',
        createdAt: 'Создано: {value}',
      },
      sort: {
        name: 'Название',
        status: 'Статус',
        ownership: 'Владение',
        createdAt: 'Создано',
      },
    },
    new: {
      title: 'Новая организация',
      subtitle: 'Создание организации в `/api/admin/organizations`.',
      saving: 'Сохраняем...',
      errors: {
        create: 'Не удалось создать организацию.',
      },
    },
    show: {
      title: 'Организация',
      subtitle: 'Show-страница организации.',
      listMode: {
        table: 'Таблица',
        cards: 'Карточки',
      },
      joinRequests: {
        title: 'Заявки на вступление',
        subtitle: 'Последние заявки пользователей и детей в эту организацию.',
        searchPlaceholder: 'Поиск: субъект, заявитель, ID, сообщение',
        filterStatus: 'Фильтр по статусу',
        filterType: 'Фильтр по типу',
        empty: 'Заявок пока нет.',
        subjectType: 'Тип субъекта',
        subject: 'Субъект',
        requestedBy: 'Кто подал',
        reviewedBy: 'Кто рассмотрел',
        reviewNote: 'Комментарий: {value}',
        headers: {
          id: 'ID',
          type: 'Тип',
          subject: 'Субъект',
          requestedBy: 'Заявитель',
          status: 'Статус',
          createdAt: 'Создано',
          actions: 'Действия',
        },
        card: {
          id: 'ID: {value}',
          type: 'Тип: {value}',
          status: 'Статус: {value}',
          requestedBy: 'Заявитель: {value}',
          createdAt: 'Создано: {value}',
        },
        actions: {
          approve: 'Одобрить',
          reject: 'Отклонить',
        },
        reviewModal: {
          title: 'Подтверждение действия',
          requestLabel: 'Заявка: {value}',
          noteLabel: 'Комментарий',
          notePlaceholder: 'Укажите причину отказа или комментарий',
          noteRequired: 'Укажите причину отказа.',
        },
        types: {
          user: 'Пользователь',
          child: 'Ребенок',
        },
        statuses: {
          pending: 'Ожидает',
          approved: 'Одобрена',
          rejected: 'Отклонена',
        },
        errors: {
          load: 'Не удалось загрузить заявки.',
          review: 'Не удалось изменить статус заявки.',
        },
      },
      members: {
        title: 'Пользователи организации',
        subtitle: 'Сотрудники и администраторы организации.',
        searchPlaceholder: 'Поиск: ФИО, email, должность, ID',
        filterStatus: 'Фильтр по статусу',
        empty: 'Пользователи не найдены.',
        position: 'Должность',
        status: 'Статус',
        headers: {
          id: 'ID',
          user: 'Пользователь',
          position: 'Должность',
          status: 'Статус',
          joinedAt: 'Вступил',
        },
        card: {
          id: 'ID: {value}',
          position: 'Должность: {value}',
          status: 'Статус: {value}',
          joinedAt: 'Вступил: {value}',
        },
        statuses: {
          active: 'Активен',
          invited: 'Приглашен',
          blocked: 'Заблокирован',
        },
        errors: {
          load: 'Не удалось загрузить пользователей организации.',
        },
      },
      clients: {
        title: 'Клиенты организации',
        subtitle: 'Родители и дети, привязанные к организации.',
        searchPlaceholder: 'Поиск: ФИО, email, ID',
        filterStatus: 'Фильтр по статусу',
        filterType: 'Фильтр по типу',
        empty: 'Клиенты не найдены.',
        subjectType: 'Тип субъекта',
        subject: 'Субъект',
        status: 'Статус',
        headers: {
          id: 'ID',
          type: 'Тип',
          subject: 'Субъект',
          status: 'Статус',
          joinedAt: 'Вступил',
        },
        card: {
          id: 'ID: {value}',
          type: 'Тип: {value}',
          status: 'Статус: {value}',
          joinedAt: 'Вступил: {value}',
        },
        statuses: {
          active: 'Активен',
          left: 'Выбыл',
          blocked: 'Заблокирован',
        },
        errors: {
          load: 'Не удалось загрузить клиентов организации.',
        },
      },
      errors: {
        invalidId: 'Некорректный идентификатор организации.',
        load: 'Не удалось загрузить организацию.',
      },
    },
    edit: {
      title: 'Редактирование организации',
      subtitle: 'Edit-страница организации.',
      saving: 'Сохраняем...',
      errors: {
        invalidId: 'Некорректный идентификатор организации.',
        load: 'Не удалось загрузить организацию.',
        update: 'Не удалось обновить организацию.',
      },
    },
    fields: {
      name: 'Название',
      description: 'Описание',
      country: 'Страна',
      region: 'Регион',
      address: 'Адрес',
      city: 'Город',
      district: 'Район',
      lat: 'Широта',
      lng: 'Долгота',
      coordinates: 'Координаты',
      metroStation: 'Станция метро',
      travelMode: 'Как добраться',
      durationMinutes: 'Минут в пути',
      walkDurationMinutes: 'Пешком, мин',
      driveDurationMinutes: 'На машине, мин',
      phone: 'Телефон',
      email: 'Email',
      status: 'Статус',
      sourceType: 'Источник',
      ownershipStatus: 'Статус владения',
      owner: 'Владелец',
      ownerUserId: 'ID владельца',
      claimedAt: 'Дата подтверждения',
      createdAt: 'Дата создания',
    },
    locations: {
      title: 'Адреса',
      subtitle: 'У организации может быть несколько адресов и несколько станций метро у каждого.',
      empty: 'Адреса еще не добавлены.',
      addLocation: 'Добавить адрес',
      removeLocation: 'Удалить адрес',
      locationTitle: 'Адрес #{index}',
      countryPlaceholder: 'Выберите страну',
      regionPlaceholder: 'Выберите регион',
      cityPlaceholder: 'Выберите город',
      districtPlaceholder: 'Выберите район',
      metroTitle: 'Метро рядом',
      addMetro: 'Добавить метро',
      removeMetro: 'Удалить метро',
      metroItemTitle: 'Метро #{index}',
      metroPlaceholder: 'Выберите станцию',
      metroEmpty: 'Станции метро не добавлены.',
      metroRequiresCity: 'Сначала выберите город, чтобы подобрать станции метро.',
      metroSummary: '#{index}: {station}, {mode}, {minutes} мин.',
    },
    ownerPlaceholder: 'Поиск пользователя: ФИО или ID',
    travelMode: {
      walk: 'Пешком',
      drive: 'На машине',
    },
    status: {
      draft: 'Черновик',
      active: 'Активна',
      suspended: 'Приостановлена',
      archived: 'Архив',
    },
    source: {
      manual: 'Вручную',
      import: 'Импорт',
      parsed: 'Парсинг',
      selfRegistered: 'Саморегистрация',
    },
    ownership: {
      unclaimed: 'Не подтверждена',
      pendingClaim: 'Ожидает подтверждения',
      claimed: 'Подтверждена',
    },
    confirmDelete: 'Удалить организацию {name}?',
    errors: {
      loadList: 'Не удалось загрузить список организаций.',
      delete: 'Не удалось удалить организацию.',
    },
  },
  roles: {
    index: {
      title: 'Роли',
      subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
      createLabel: 'Новая роль',
      searchPlaceholder: 'Поиск: code или label',
      empty: 'Роли не найдены.',
      headers: {
        code: 'Code',
        label: 'Label',
        type: 'Тип',
        actions: 'Действия',
      },
      sort: {
        code: 'Code',
        label: 'Label',
        type: 'Тип',
      },
      type: {
        system: 'Системная',
        custom: 'Пользовательская',
      },
      cardLabelFallback: 'Без label',
    },
    new: {
      title: 'Новая роль',
      subtitle: 'Создание роли в `/api/admin/roles`.',
      fields: {
        code: 'Code',
        label: 'Label',
      },
      saving: 'Сохраняем...',
      errors: {
        create: 'Не удалось создать роль.',
      },
    },
    show: {
      title: 'Роль',
      subtitle: 'Show-страница роли.',
      labels: {
        code: 'Code',
        label: 'Label',
        type: 'Тип',
      },
      errors: {
        invalidId: 'Некорректный идентификатор роли.',
        load: 'Не удалось загрузить роль.',
      },
    },
    edit: {
      title: 'Редактирование роли',
      subtitle: 'Edit-страница роли.',
      fields: {
        code: 'Code',
        label: 'Label',
      },
      systemLocked: 'Системные роли редактировать нельзя.',
      saving: 'Сохраняем...',
      errors: {
        invalidId: 'Некорректный идентификатор роли.',
        load: 'Не удалось загрузить роль.',
        update: 'Не удалось обновить роль.',
      },
    },
    errors: {
      loadList: 'Не удалось загрузить роли.',
      delete: 'Не удалось удалить роль.',
    },
    confirmDelete: 'Удалить роль {code}?',
  },
  geo: {
    title: 'География',
    countries: {
      index: {
        title: 'Страны',
        subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
        createLabel: 'Новая страна',
        searchPlaceholder: 'Поиск: название, ISO код',
        empty: 'Страны не найдены.',
        headers: {
          name: 'Название',
          isoCode: 'ISO код',
          actions: 'Действия',
        },
        card: {
          isoCode: 'ISO код: {value}',
        },
        sort: {
          name: 'Название',
          isoCode: 'ISO код',
        },
      },
      new: {
        title: 'Новая страна',
        subtitle: 'Создание страны в /api/admin/geo/countries',
        saving: 'Сохраняем...',
        errors: {
          create: 'Не удалось создать страну.',
        },
      },
      show: {
        title: 'Страна',
        subtitle: 'Детали страны',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить страну.',
        },
      },
      edit: {
        title: 'Редактирование страны',
        subtitle: 'Обновление /api/admin/geo/countries/:id',
        saving: 'Сохраняем...',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить страну.',
          update: 'Не удалось обновить страну.',
        },
      },
      fields: {
        name: 'Название',
        isoCode: 'ISO код',
      },
      confirmDelete: 'Удалить страну {name}?',
      errors: {
        loadList: 'Не удалось загрузить список стран.',
        delete: 'Не удалось удалить страну.',
      },
    },
    regions: {
      index: {
        title: 'Регионы',
        subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
        createLabel: 'Новый регион',
        searchPlaceholder: 'Поиск: название, страна',
        empty: 'Регионы не найдены.',
        headers: {
          name: 'Название',
          countryId: 'Страна',
          actions: 'Действия',
        },
        card: {
          countryId: 'Страна: {value}',
        },
        sort: {
          name: 'Название',
          countryId: 'Страна',
        },
      },
      new: {
        title: 'Новый регион',
        subtitle: 'Создание региона в /api/admin/geo/regions',
        saving: 'Сохраняем...',
        errors: {
          create: 'Не удалось создать регион.',
        },
      },
      show: {
        title: 'Регион',
        subtitle: 'Детали региона',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить регион.',
        },
      },
      edit: {
        title: 'Редактирование региона',
        subtitle: 'Обновление /api/admin/geo/regions/:id',
        saving: 'Сохраняем...',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить регион.',
          update: 'Не удалось обновить регион.',
        },
      },
      fields: {
        name: 'Название',
        countryId: 'ID страны',
        country: 'Страна',
      },
      confirmDelete: 'Удалить регион {name}?',
      errors: {
        loadList: 'Не удалось загрузить список регионов.',
        delete: 'Не удалось удалить регион.',
      },
    },
    cities: {
      index: {
        title: 'Города',
        subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
        createLabel: 'Новый город',
        searchPlaceholder: 'Поиск: название, страна, регион',
        empty: 'Города не найдены.',
        headers: {
          name: 'Название',
          countryId: 'Страна',
          regionId: 'Регион',
          actions: 'Действия',
        },
        card: {
          countryId: 'Страна: {value}',
          regionId: 'Регион: {value}',
        },
        sort: {
          name: 'Название',
          countryId: 'Страна',
          regionId: 'Регион',
        },
      },
      new: {
        title: 'Новый город',
        subtitle: 'Создание города в /api/admin/geo/cities',
        saving: 'Сохраняем...',
        errors: {
          create: 'Не удалось создать город.',
        },
      },
      show: {
        title: 'Город',
        subtitle: 'Детали города',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить город.',
        },
      },
      edit: {
        title: 'Редактирование города',
        subtitle: 'Обновление /api/admin/geo/cities/:id',
        saving: 'Сохраняем...',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить город.',
          update: 'Не удалось обновить город.',
        },
      },
      fields: {
        name: 'Название',
        countryId: 'ID страны',
        regionId: 'ID региона',
        country: 'Страна',
        region: 'Регион',
      },
      confirmDelete: 'Удалить город {name}?',
      errors: {
        loadList: 'Не удалось загрузить список городов.',
        delete: 'Не удалось удалить город.',
      },
    },
    districts: {
      index: {
        title: 'Районы',
        subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
        createLabel: 'Новый район',
        searchPlaceholder: 'Поиск: название, город',
        empty: 'Районы не найдены.',
        headers: {
          name: 'Название',
          cityId: 'Город',
          actions: 'Действия',
        },
        card: {
          city: 'Город: {value}',
        },
        sort: {
          name: 'Название',
          cityId: 'Город',
        },
      },
      new: {
        title: 'Новый район',
        subtitle: 'Создание района в /api/admin/geo/districts',
        saving: 'Сохраняем...',
        errors: {
          create: 'Не удалось создать район.',
        },
      },
      show: {
        title: 'Район',
        subtitle: 'Детали района',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить район.',
        },
      },
      edit: {
        title: 'Редактирование района',
        subtitle: 'Обновление /api/admin/geo/districts/:id',
        saving: 'Сохраняем...',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить район.',
          update: 'Не удалось обновить район.',
        },
      },
      fields: {
        name: 'Название',
        cityId: 'ID города',
        city: 'Город',
      },
      confirmDelete: 'Удалить район {name}?',
      errors: {
        loadList: 'Не удалось загрузить список районов.',
        delete: 'Не удалось удалить район.',
      },
    },
  },
  metro: {
    title: 'Метро',
    lines: {
      index: {
        title: 'Линии метро',
        subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
        createLabel: 'Новая линия',
        searchPlaceholder: 'Поиск: название, ID линии, город',
        empty: 'Линии метро не найдены.',
        headers: {
          name: 'Название',
          lineId: 'ID линии',
          color: 'Цвет',
          cityId: 'Город',
          actions: 'Действия',
        },
        card: {
          lineId: 'ID линии: {value}',
          color: 'Цвет:',
          cityId: 'Город: {value}',
        },
        sort: {
          name: 'Название',
          lineId: 'ID линии',
          cityId: 'Город',
        },
      },
      new: {
        title: 'Новая линия метро',
        subtitle: 'Создание линии в /api/admin/metro-lines',
        saving: 'Сохраняем...',
        errors: {
          create: 'Не удалось создать линию метро.',
        },
      },
      show: {
        title: 'Линия метро',
        subtitle: 'Детали линии метро',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить линию метро.',
        },
      },
      edit: {
        title: 'Редактирование линии метро',
        subtitle: 'Обновление /api/admin/metro-lines/:id',
        saving: 'Сохраняем...',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить линию метро.',
          update: 'Не удалось обновить линию метро.',
        },
      },
      fields: {
        name: 'Название',
        externalId: 'Внешний ID',
        lineId: 'ID линии',
        color: 'Цвет',
        cityId: 'ID города',
        city: 'Город',
        source: 'Источник',
      },
      confirmDelete: 'Удалить линию метро {name}?',
      errors: {
        loadList: 'Не удалось загрузить список линий метро.',
        delete: 'Не удалось удалить линию метро.',
      },
    },
    stations: {
      index: {
        title: 'Станции метро',
        subtitle: 'Поиск, сортировка, limit и серверная пагинация.',
        createLabel: 'Новая станция',
        searchPlaceholder: 'Поиск: название, line ID, линия метро, city ID',
        empty: 'Станции метро не найдены.',
        headers: {
          name: 'Название',
          lineId: 'ID линии',
          metroLine: 'Линия метро',
          city: 'Город',
          actions: 'Действия',
        },
        card: {
          lineId: 'ID линии: {value}',
          metroLine: 'Линия метро: {value}',
          city: 'Город: {value}',
        },
        sort: {
          name: 'Название',
          lineId: 'ID линии',
          metroLine: 'Линия метро',
          city: 'Город',
        },
      },
      new: {
        title: 'Новая станция метро',
        subtitle: 'Создание станции в /api/admin/metro-stations',
        saving: 'Сохраняем...',
        metroLinePlaceholder: 'Выберите линию метро',
        cityPlaceholder: 'Выберите город',
        errors: {
          create: 'Не удалось создать станцию метро.',
        },
      },
      show: {
        title: 'Станция метро',
        subtitle: 'Детали станции метро',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить станцию метро.',
        },
      },
      edit: {
        title: 'Редактирование станции метро',
        subtitle: 'Обновление /api/admin/metro-stations/:id',
        saving: 'Сохраняем...',
        metroLinePlaceholder: 'Выберите линию метро',
        cityPlaceholder: 'Выберите город',
        errors: {
          invalidId: 'Некорректный ID.',
          load: 'Не удалось загрузить станцию метро.',
          update: 'Не удалось обновить станцию метро.',
        },
      },
      fields: {
        name: 'Название',
        externalId: 'Внешний ID',
        lineId: 'ID линии',
        geo: 'Координаты',
        geoLat: 'Широта',
        geoLon: 'Долгота',
        isClosed: 'Станция закрыта',
        metroLine: 'Линия метро',
        metroLineId: 'ID линии метро',
        cityId: 'ID города',
        source: 'Источник',
      },
      status: {
        closedYes: 'Да',
        closedNo: 'Нет',
      },
      confirmDelete: 'Удалить станцию метро {name}?',
      errors: {
        loadList: 'Не удалось загрузить список станций метро.',
        delete: 'Не удалось удалить станцию метро.',
      },
    },
  },
  actionLogs: {
    title: 'Журнал изменений',
    subtitle: 'История изменений моделей (создание, обновление, удаление).',
    empty: 'Записей пока нет.',
    headers: {
      createdAt: 'Дата',
      event: 'Событие',
      model: 'Сущность',
      user: 'Автор',
      changedFields: 'Изменённые поля',
      ip: 'IP',
    },
    events: {
      create: 'Создание',
      update: 'Обновление',
      delete: 'Удаление',
    },
    models: {
      user: 'Пользователь',
      role: 'Роль',
      child: 'Ребенок',
      organization: 'Организация',
      metroLine: 'Линия метро',
      metroStation: 'Станция метро',
      geoCountry: 'Страна',
      geoRegion: 'Регион',
      geoCity: 'Город',
      geoDistrict: 'Район',
    },
    filters: {
      event: 'Событие',
      anyEvent: 'Все события',
      model: 'Сущность',
      anyModel: 'Все сущности',
      user: 'Пользователь',
      userPlaceholder: 'ID / email / ФИО',
      dateRange: 'Диапазон дат',
      dateFrom: 'Дата от',
      dateTo: 'Дата до',
      searchPlaceholder: 'Поиск по сущности/событию/id',
      eventAll: 'Все',
      eventCreate: 'Создание',
      eventUpdate: 'Обновление',
      eventDelete: 'Удаление',
      modelAll: 'Все',
      modelUser: 'Пользователь',
      modelRole: 'Роль',
      modelChild: 'Ребенок',
      modelOrganization: 'Организация',
      modelMetroLine: 'Линия метро',
      modelMetroStation: 'Станция метро',
      modelGeoCountry: 'Страна',
      modelGeoRegion: 'Регион',
      modelGeoCity: 'Город',
      modelGeoDistrict: 'Район',
    },
    errors: {
      load: 'Не удалось загрузить журнал действий.',
    },
  },
  errors: {
    users: {
      loadList: 'Не удалось загрузить пользователей.',
      delete: 'Не удалось удалить пользователя.',
    },
  },
} as const;
