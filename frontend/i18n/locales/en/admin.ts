export default {
  layout: {
    panelTitle: 'Admin Panel',
    shortPanelTitle: 'AP',
    closeSidebar: 'Close menu',
    openSidebar: 'Open menu',
    sidebarToggleExpand: 'Expand menu',
    sidebarToggleCollapse: 'Collapse menu',
    heading: 'Administration panel',
    breadcrumbsAria: 'Breadcrumbs',
    toggleLightMode: 'Toggle light mode',
    toggleDarkMode: 'Toggle dark mode',
    menu: {
      dashboard: 'Dashboard',
      settings: 'Settings',
      users: 'Users',
      roles: 'Roles',
      categories: 'Categories',
      activities: 'Activities',
      activityLeads: 'Activity leads',
      organizations: 'Organizations',
      children: 'Children',
      actionLogs: 'Change log',
      monitoring: 'Monitoring',
      metroLines: 'Lines',
      metroStations: 'Stations',
      geoCountries: 'Countries',
      geoRegions: 'Regions',
      geoCities: 'Cities',
      geoDistricts: 'Districts',
    },
    sections: {
      system: 'System',
      geo: 'Geography',
      metro: 'Metro',
      organization: 'Organization',
    },
    user: {
      guest: 'Guest',
      noEmail: 'No email',
    },
  },
  userMenu: {
    profile: 'Profile',
    settings: 'Settings',
    logout: 'Logout',
  },
  entity: {
    shownCount: 'Showing {shown} of {total}.',
    desktopTable: 'Desktop: table',
    desktopCards: 'Desktop: cards',
    modePlaceholder: 'Table',
    modes: {
      table: 'Table',
      tableCards: 'Table + cards',
      cards: 'Cards',
    },
  },
  toolbar: {
    search: 'Search',
    perPage: '{count} per page',
    find: 'Find',
    reset: 'Reset',
  },
  pagination: {
    summary: 'Page {current} / {last}. Per page: {perPage}.',
    back: 'Back',
    forward: 'Next',
  },
  actions: {
    show: 'Show',
    edit: 'Edit',
    delete: 'Delete',
    rollback: 'Rollback',
  },
  files: {
    avatarHint: 'PNG/JPG/WEBP, up to 5MB',
  },
  permissions: {
    title: 'Role permissions',
    roleHint: 'Select permissions granted by default for this role.',
    userTitle: 'User personal permissions',
    userHint:
      'Allow grants a specific permission, Deny blocks a specific permission (overrides roles).',
    allow: 'Allow',
    deny: 'Deny',
    scopes: {
      admin: 'Admin',
      org: 'Organization',
      user: 'User',
    },
    codes: {
      admin: {
        panel: {
          access: 'Admin panel access',
        },
        users: {
          read: 'Read users in admin',
          create: 'Create users in admin',
          update: 'Update users in admin',
          delete: 'Delete users in admin',
        },
        roles: {
          read: 'Read roles in admin',
          create: 'Create roles in admin',
          update: 'Update roles in admin',
          delete: 'Delete roles in admin',
        },
        changelog: {
          read: 'Read changelog in admin',
          rollback: 'Request changelog rollback in admin',
        },
        action_log: {
          read: 'Read action log in admin',
          update: 'Update action log in admin',
          delete: 'Delete action log in admin',
        },
        monitoring: {
          read: 'Read monitoring in admin',
        },
        activity_leads: {
          read: 'View activity leads in admin panel',
          update: 'Update activity lead statuses in admin panel',
        },
      },
      org: {
        company: {
          profile: {
            read: 'Read company account',
            update: 'Update company account',
            delete: 'Delete company account',
          },
        },
        members: {
          read: 'Read organization members',
          write: 'Manage organization members',
        },
        children: {
          read: 'Read children in organization',
          write: 'Manage children in organization',
        },
        activity_leads: {
          read: 'View activity leads in organization workspace',
          update: 'Update activity lead statuses in organization workspace',
        },
      },
      user: {
        profile: {
          read: 'Read user account',
          update: 'Update user account',
          delete: 'Delete user account',
        },
      },
    },
  },
  changelog: {
    title: 'Change history',
    subtitle: 'Audit trail for the current entity.',
    version: 'Version',
    actor: 'Actor',
    actorMe: 'Me',
    changedFields: 'Changed fields',
    was: 'was',
    now: 'now',
    set: 'set',
    complexChanged: 'changed',
    showJson: 'Show JSON',
    hideJson: 'Hide JSON',
    createdData: 'Created',
    mediaSnapshots: 'Media snapshots',
    before: 'Before',
    after: 'After',
    preview: {
      title: 'Image',
      open: 'Open image',
    },
    empty: 'No changes yet.',
    rollback: {
      title: 'Rollback changes',
      confirm: 'Rollback to version #{version} ({event})?',
      fromVersion: 'Rolled back from version #{version}',
    },
    fields: {
      common: {
        settings: 'Settings',
        locale: 'Locale',
        theme: 'Theme',
        collapse_menu: 'Collapse menu',
      },
      profile: {
        first_name: 'First name',
        last_name: 'Last name',
        middle_name: 'Middle name',
        gender: 'Gender',
        email: 'Email',
        phone: 'Phone',
        birth_date: 'Birth date',
      },
      user: {
        first_name: 'First name',
        last_name: 'Last name',
        middle_name: 'Middle name',
        gender: 'Gender',
        email: 'Email',
        phone: 'Phone',
        birth_date: 'Birth date',
        roles: 'Roles',
        avatar_id: 'Avatar',
        permission_overrides: 'Personal permissions',
      },
      role: {
        code: 'Code',
        label: 'Label',
        is_system: 'System role',
      },
      child: {
        first_name: 'First name',
        last_name: 'Last name',
        middle_name: 'Middle name',
        gender: 'Gender',
        birth_date: 'Birth date',
        user_id: 'User',
      },
      organization: {
        name: 'Name',
        description: 'Description',
        address: 'Address',
        phone: 'Phone',
        email: 'Email',
        status: 'Status',
        source_type: 'Source',
        ownership_status: 'Ownership status',
        owner_user_id: 'Owner',
      },
    },
    events: {
      create: 'Create',
      update: 'Update',
      delete: 'Delete',
      restore: 'Restore',
    },
    errors: {
      load: 'Failed to load change history.',
      rollback: 'Failed to rollback changes.',
    },
  },
  dashboard: {
    title: 'Dashboard',
    subtitle: 'Select a module in the left menu. The selected section content is shown here.',
    usersTitle: 'Users',
    usersSubtitle: 'Manage roles, accesses, and admin profiles.',
    contentTitle: 'Content',
    contentSubtitle: 'Edit sections, configure visibility, and moderate.',
    stats: {
      totalUsers: 'Total users',
      onlineUsers: 'Online now',
      onlineHint: 'Updated in real time',
      loadError: 'Failed to load users statistics.',
    },
  },
  categories: {
    fields: {
      name: 'Name',
      slug: 'Slug',
      parent: 'Parent category',
      sortOrder: 'Sort order',
      isActive: 'Active category',
    },
    index: {
      title: 'Categories',
      subtitle: 'Manage the category tree and category statuses.',
      createLabel: 'New category',
      searchPlaceholder: 'Search categories by name or slug',
      empty: 'No categories yet.',
      root: 'Root category',
      headers: {
        name: 'Name',
        slug: 'Slug',
        parent: 'Parent',
        sortOrder: 'Order',
        status: 'Status',
        children: 'Children',
        activities: 'Activities',
        actions: 'Actions',
      },
      card: {
        slug: 'Slug: {value}',
        parent: 'Parent: {value}',
        sortOrder: 'Order: {value}',
        status: 'Status: {value}',
        children: 'Children: {value}',
        activities: 'Activities: {value}',
      },
      filteredByParent: 'Showing child categories for: {value}',
      clearParentFilter: 'Show all categories',
      sort: {
        name: 'Name',
        slug: 'Slug',
        sortOrder: 'Order',
        status: 'Status',
      },
    },
    status: {
      active: 'Active',
      inactive: 'Inactive',
    },
    new: {
      title: 'New category',
      subtitle: 'Create a root category or a child category.',
      parentPlaceholder: 'No parent',
      isActiveHint: 'Inactive categories are hidden from public navigation.',
      slugAutofill: 'Auto',
      slugHint: 'You can edit the slug manually if the generated version is not suitable.',
      slugLoading: 'Generating slug...',
      saving: 'Creating...',
      errors: {
        create: 'Failed to create category.',
        loadParents: 'Failed to load parent categories.',
        previewSlug: 'Failed to generate slug.',
      },
    },
    show: {
      title: 'Category',
      subtitle: 'View category details and its child nodes.',
      labels: {
        children: 'Child categories',
        activities: 'Linked activities',
      },
      childrenTitle: 'Child categories',
      openChildren: 'Open children',
      errors: {
        invalidId: 'Invalid category id.',
        load: 'Failed to load category.',
      },
    },
    edit: {
      title: 'Edit category',
      subtitle: 'Update the category name, slug, and its position in the tree.',
      parentPlaceholder: 'No parent',
      isActiveHint: 'Inactive categories are hidden from public navigation.',
      slugAutofill: 'Auto',
      slugHint: 'You can edit the slug manually if the generated version is not suitable.',
      slugLoading: 'Generating slug...',
      saving: 'Saving...',
      errors: {
        invalidId: 'Invalid category id.',
        load: 'Failed to load category.',
        update: 'Failed to save category.',
        loadParents: 'Failed to load parent categories.',
        previewSlug: 'Failed to generate slug.',
      },
    },
    confirmDelete: 'Delete category "{name}"?',
    errors: {
      loadList: 'Failed to load categories.',
      delete: 'Failed to delete category.',
    },
  },
  activities: {
    fields: {
      organization: 'Organization',
      location: 'Location',
      category: 'Category',
      rootCategory: 'Primary category',
      subcategory: 'Subcategory',
      name: 'Name',
      slug: 'Slug',
      shortDescription: 'Short description',
      description: 'Description',
      minAge: 'Min age',
      maxAge: 'Max age',
      capacity: 'Capacity',
      priceFrom: 'Price from',
      priceTo: 'Price to',
      currency: 'Currency',
      status: 'Status',
      publishedAt: 'Published at',
      isFeatured: 'Show in featured blocks',
    },
    index: {
      title: 'Activities',
      subtitle: 'Entry point for managing sections, clubs, and other activities.',
      createLabel: 'New activity',
      searchPlaceholder: 'Search activities by name, slug, or organization',
      empty: 'No activities yet.',
      headers: {
        name: 'Name',
        organization: 'Organization',
        location: 'Location',
        category: 'Category',
        status: 'Status',
        featured: 'Featured',
        publishedAt: 'Published',
        actions: 'Actions',
      },
      card: {
        organization: 'Organization: {value}',
        location: 'Location: {value}',
        category: 'Category: {value}',
        status: 'Status: {value}',
        featured: 'Featured: {value}',
        publishedAt: 'Published: {value}',
      },
      sort: {
        createdAt: 'Created',
        name: 'Name',
        status: 'Status',
        publishedAt: 'Published',
        featured: 'Featured',
      },
    },
    new: {
      title: 'New activity',
      subtitle: 'Create a new section, club, or other activity.',
      organizationPlaceholder: 'Select organization',
      locationPlaceholder: 'Select organization first',
      rootCategoryPlaceholder: 'Select root category',
      categoryPlaceholder: 'Select leaf subcategory',
      locationLabel: 'Location',
      statusPlaceholder: 'Select status',
      isFeaturedHint: 'Featured activities may appear in featured blocks.',
      saving: 'Creating...',
      errors: {
        create: 'Failed to create activity.',
        loadOrganizations: 'Failed to load organizations.',
        loadCategories: 'Failed to load categories.',
        rootCategoryRequired: 'Select a primary category.',
        subcategoryRequired: 'Select a subcategory.',
      },
    },
    show: {
      title: 'Activity',
      subtitle: 'View activity details and linked data.',
      schedulesTitle: 'Schedule',
      scheduleTime: '{start} - {end}',
      coverTitle: 'Cover',
      galleryTitle: 'Gallery',
      errors: {
        invalidId: 'Invalid activity identifier.',
        load: 'Failed to load activity.',
      },
    },
    edit: {
      title: 'Edit activity',
      subtitle: 'Update activity details and core parameters.',
      slugAutofill: 'Auto',
      slugHint: 'Slug can be edited manually; autofill will not overwrite it.',
      slugLoading: 'Generating slug...',
      saving: 'Saving...',
      errors: {
        invalidId: 'Invalid activity identifier.',
        load: 'Failed to load activity.',
        update: 'Failed to update activity.',
        previewSlug: 'Failed to generate slug.',
        rootCategoryRequired: 'Select a primary category.',
        subcategoryRequired: 'Select a subcategory.',
      },
    },
    filters: {
      status: 'Status',
      featured: 'Featured',
      allStatuses: 'All statuses',
      allFeatured: 'All activities',
      featuredOnly: 'Featured only',
      notFeatured: 'Not featured',
    },
    status: {
      draft: 'Draft',
      pendingReview: 'Pending review',
      published: 'Published',
      archived: 'Archived',
    },
    schedule: {
      title: 'Schedule',
      subtitle: 'Add weekday and time slots for the activity.',
      add: 'Add slot',
      empty: 'No schedule slots added yet.',
      weekday: 'Weekday',
      weekdayPlaceholder: 'Select day',
      startTime: 'Start time',
      endTime: 'End time',
      weekdays: {
        monday: 'Monday',
        tuesday: 'Tuesday',
        wednesday: 'Wednesday',
        thursday: 'Thursday',
        friday: 'Friday',
        saturday: 'Saturday',
        sunday: 'Sunday',
      },
    },
    media: {
      title: 'Photos',
      subtitle: 'Upload activity cover and gallery images.',
      imageHint: 'PNG, JPEG, and WEBP up to 10MB are supported.',
      coverTitle: 'Cover',
      coverHint: 'Primary image for the activity card.',
      coverEmpty: 'No cover uploaded yet.',
      coverUploadTitle: 'Upload cover',
      coverUploadButton: 'Choose cover',
      coverExistingCaption: 'Current cover',
      coverDraftCaption: 'New cover',
      galleryTitle: 'Gallery',
      galleryHint: 'Additional activity photos.',
      galleryEmpty: 'No gallery images uploaded yet.',
      galleryUploadTitle: 'Upload photos',
      galleryUploadButton: 'Choose photos',
      galleryExistingCaption: 'Current photo',
      galleryDraftCaption: 'New photo',
    },
    labels: {
      featuredYes: 'Yes',
      featuredNo: 'No',
    },
    confirmDelete: 'Delete activity "{name}"?',
    errors: {
      loadList: 'Failed to load activities list.',
      delete: 'Failed to delete activity.',
    },
  },
  activityLeads: {
    index: {
      title: 'Activity leads',
      subtitle: 'Global inbound queue for sections, clubs, and other activity requests.',
      searchPlaceholder: 'Search: activity, subject, message, email, phone',
      empty: 'No activity leads yet.',
      headers: {
        activity: 'Activity',
        subject: 'Subject',
        status: 'Status',
        requestType: 'Request type',
        channels: 'Contact channels',
        message: 'Message',
        createdAt: 'Created at',
        actions: 'Actions',
      },
      card: {
        status: 'Status: {value}',
        requestType: 'Request type: {value}',
        channels: 'Channels: {value}',
        createdAt: 'Created at: {value}',
      },
      sort: {
        createdAt: 'Created',
        status: 'Status',
      },
      actions: {
        saveStatus: 'Save status',
      },
    },
    filters: {
      status: 'Status',
      requestType: 'Request type',
      allStatuses: 'All statuses',
      allRequestTypes: 'All request types',
    },
    status: {
      new: 'New',
      inProgress: 'In progress',
      contacted: 'Contacted',
      registered: 'Registered',
      cancelled: 'Cancelled',
    },
    requestType: {
      self: 'For self',
      child: 'For child',
    },
    errors: {
      loadList: 'Failed to load activity leads.',
      updateStatus: 'Failed to update lead status.',
    },
  },
  monitoring: {
    title: 'Monitoring',
    subtitle: 'Observability summary and incidents by domains.',
    filters: {
      domain: 'Domain',
      allDomains: 'All domains',
      incidentsLimit: 'Incidents limit',
      refresh: 'Refresh',
    },
    cards: {
      eventsTotal: 'Events',
      errorsTotal: 'Errors',
      avgDuration: 'Average duration (ms)',
      lastEventAt: 'Last event',
    },
    kpi: {
      ok: 'OK',
      total: 'Total',
      chartTitle: 'Events by domain',
      chartYLabel: 'Value',
      seriesEvents: 'Events',
      seriesErrors: 'Errors',
      seriesAvgDuration: 'Average duration',
      errorRate: 'Error rate: {value}%',
      domainsCount: 'Domains: {count}',
      lastEvent: 'Last event: {value}',
      msValue: '{value} ms',
    },
    incidents: {
      title: 'Recent incidents',
      empty: 'No incidents found.',
      headers: {
        time: 'Time',
        domain: 'Domain',
        event: 'Event',
        status: 'Status',
        severity: 'Severity',
        component: 'Component',
      },
    },
    alerts: {
      title: 'Alerts',
      empty: 'No active alerts.',
    },
    errors: {
      load: 'Failed to load monitoring data.',
    },
  },
  profile: {
    title: 'My profile',
    subtitle: 'Edit personal data of the current user.',
    avatar: {
      title: 'Avatar',
      upload: 'Upload',
      delete: 'Delete avatar',
      previewAlt: 'User avatar',
      errors: {
        upload: 'Failed to upload avatar.',
        delete: 'Failed to delete avatar.',
      },
    },
    fields: {
      firstName: 'First name',
      lastName: 'Last name',
      middleName: 'Middle name',
      gender: 'Gender',
      phone: 'Phone',
      birthDate: 'Birth date',
      email: 'Email',
    },
    genderPlaceholder: 'Select gender',
    saving: 'Saving...',
    errors: {
      update: 'Failed to update profile.',
    },
  },
  settings: {
    title: 'User settings',
    subtitle: 'Settings are saved automatically in browser and synced with backend.',
    theme: {
      label: 'Dark theme',
      description: 'Switches interface color scheme',
      hint: 'Stored in user profile',
    },
    menu: {
      label: 'Collapse menu',
      description: 'Collapses sidebar to icons-only mode',
      hint: 'Controls admin sidebar width',
    },
  },
  genders: {
    male: 'Male',
    female: 'Female',
  },
  users: {
    index: {
      title: 'Users',
      subtitle: 'View, search, and edit all system users.',
      createLabel: 'New user',
      searchPlaceholder: 'Search: ID, last name, first name, middle name, email, phone, role',
      empty: 'No users found.',
      headers: {
        id: 'ID',
        thumbnail: 'Avatar',
        lastName: 'Last name',
        firstName: 'First name',
        middleName: 'Middle name',
        gender: 'Gender',
        access: 'Access',
        status: 'Status',
        actions: 'Actions',
      },
      preview: {
        title: 'Image',
        open: 'Open image',
      },
      card: {
        id: 'ID: {value}',
        lastName: 'Last name: {value}',
        firstName: 'First name: {value}',
        middleName: 'Middle name: {value}',
        gender: 'Gender: {value}',
        status: 'Status: {value}',
      },
      sort: {
        id: 'ID',
        lastName: 'Last name',
        firstName: 'First name',
        middleName: 'Middle name',
        gender: 'Gender',
        access: 'Access',
      },
      access: {
        unknown: 'Unknown',
        admin: 'Admin panel',
        basic: 'No admin access',
      },
      tags: {
        admin: 'Admins',
        basic: 'Regular users',
      },
    },
    new: {
      title: 'New user',
      subtitle: 'Create user in `/api/admin/users`.',
      fields: {
        firstName: 'First name',
        lastName: 'Last name',
        middleName: 'Middle name',
        gender: 'Gender',
        birthDate: 'Birth date',
        email: 'Email',
        phone: 'Phone',
        password: 'Password',
        passwordConfirmation: 'Password confirmation',
        roles: 'Roles',
        avatar: 'Avatar',
      },
      genderPlaceholder: 'Select gender',
      rolesPlaceholder: 'Select roles',
      saving: 'Saving...',
      errors: {
        create: 'Failed to create user.',
      },
    },
    show: {
      title: 'User profile',
      subtitle: 'User show page.',
      labels: {
        firstName: 'First name',
        lastName: 'Last name',
        middleName: 'Middle name',
        gender: 'Gender',
        email: 'Email',
        phone: 'Phone',
        birthDate: 'Birth date',
        roles: 'Roles',
        status: 'Status',
      },
      errors: {
        invalidId: 'Invalid user identifier.',
        load: 'Failed to load user.',
      },
    },
    edit: {
      title: 'Edit user',
      subtitle: 'User edit page.',
      fields: {
        firstName: 'First name',
        lastName: 'Last name',
        middleName: 'Middle name',
        gender: 'Gender',
        birthDate: 'Birth date',
        email: 'Email',
        phone: 'Phone',
        newPassword: 'New password',
        newPasswordConfirmation: 'New password confirmation',
        roles: 'Roles',
        avatar: 'Avatar',
      },
      genderPlaceholder: 'Select gender',
      rolesPlaceholder: 'Select roles',
      saving: 'Saving...',
      errors: {
        invalidId: 'Invalid user identifier.',
        load: 'Failed to load user.',
        cannotEditSuperAdmin: 'Only super_admin can edit a user with the super_admin role.',
        cannotEditHigherRole: 'You cannot edit a user with a higher role than yours.',
        update: 'Failed to update user.',
      },
    },
    confirmDelete: 'Delete user {name}?',
    presence: {
      online: 'Online',
      lastSeen: 'Last seen {value}',
    },
  },
  children: {
    index: {
      title: 'Children',
      subtitle: 'Children CRUD in admin.',
      createLabel: 'New child',
      searchPlaceholder: 'Search: child full name or user ID',
      empty: 'No children found.',
      headers: {
        fullName: 'Full name',
        birthDate: 'Birth date',
        gender: 'Gender',
        user: 'Parent',
        userId: 'User ID',
        actions: 'Actions',
      },
      card: {
        birthDate: 'Birth date: {value}',
        gender: 'Gender: {value}',
        user: 'Parent: {value}',
        userId: 'User ID: {value}',
      },
      sort: {
        firstName: 'First name',
        lastName: 'Last name',
        middleName: 'Middle name',
        gender: 'Gender',
        birthDate: 'Birth date',
        user: 'Parent',
        userId: 'User ID',
      },
    },
    new: {
      title: 'New child',
      subtitle: 'Create child record in `/api/admin/children`.',
      fields: {
        firstName: 'First name',
        lastName: 'Last name',
        middleName: 'Middle name',
        gender: 'Gender',
        birthDate: 'Birth date',
        userId: 'User ID',
      },
      genderPlaceholder: 'Select gender',
      userPlaceholder: 'Search user: full name or ID',
      saving: 'Saving...',
      errors: {
        create: 'Failed to create child.',
      },
    },
    show: {
      title: 'Child profile',
      subtitle: 'Child show page.',
      labels: {
        fullName: 'Full name',
        birthDate: 'Birth date',
        gender: 'Gender',
        user: 'User',
        userId: 'User ID',
      },
      errors: {
        invalidId: 'Invalid child identifier.',
        load: 'Failed to load child.',
      },
    },
    edit: {
      title: 'Edit child',
      subtitle: 'Child edit page.',
      fields: {
        firstName: 'First name',
        lastName: 'Last name',
        middleName: 'Middle name',
        gender: 'Gender',
        birthDate: 'Birth date',
        userId: 'User ID',
      },
      genderPlaceholder: 'Select gender',
      userPlaceholder: 'Search user: full name or ID',
      saving: 'Saving...',
      errors: {
        invalidId: 'Invalid child identifier.',
        load: 'Failed to load child.',
        update: 'Failed to update child.',
      },
    },
    confirmDelete: 'Delete child record {name}?',
    errors: {
      loadList: 'Failed to load children list.',
      delete: 'Failed to delete child.',
    },
  },
  organizations: {
    index: {
      title: 'Organizations',
      subtitle: 'Organizations CRUD in admin.',
      createLabel: 'New organization',
      searchPlaceholder: 'Search: name, email, phone, ID',
      empty: 'No organizations found.',
      headers: {
        name: 'Name',
        status: 'Status',
        ownership: 'Ownership',
        owner: 'Owner',
        createdAt: 'Created',
        actions: 'Actions',
      },
      card: {
        status: 'Status: {value}',
        ownership: 'Ownership: {value}',
        owner: 'Owner: {value}',
        createdAt: 'Created: {value}',
      },
      sort: {
        name: 'Name',
        status: 'Status',
        ownership: 'Ownership',
        createdAt: 'Created',
      },
    },
    new: {
      title: 'New organization',
      subtitle: 'Create organization in `/api/admin/organizations`.',
      saving: 'Saving...',
      errors: {
        create: 'Failed to create organization.',
      },
    },
    show: {
      title: 'Organization',
      subtitle: 'Organization show page.',
      listMode: {
        table: 'Table',
        cards: 'Cards',
      },
      joinRequests: {
        title: 'Join Requests',
        subtitle: 'Latest requests from users and children for this organization.',
        searchPlaceholder: 'Search: subject, requester, ID, message',
        filterStatus: 'Filter by status',
        filterType: 'Filter by type',
        empty: 'No join requests yet.',
        subjectType: 'Subject type',
        subject: 'Subject',
        requestedBy: 'Requested by',
        reviewedBy: 'Reviewed by',
        reviewNote: 'Comment: {value}',
        headers: {
          id: 'ID',
          type: 'Type',
          subject: 'Subject',
          requestedBy: 'Requester',
          status: 'Status',
          createdAt: 'Created at',
          actions: 'Actions',
        },
        card: {
          id: 'ID: {value}',
          type: 'Type: {value}',
          status: 'Status: {value}',
          requestedBy: 'Requester: {value}',
          createdAt: 'Created at: {value}',
        },
        actions: {
          approve: 'Approve',
          reject: 'Reject',
        },
        reviewModal: {
          title: 'Confirm action',
          requestLabel: 'Request: {value}',
          noteLabel: 'Comment',
          notePlaceholder: 'Provide rejection reason or comment',
          noteRequired: 'Please provide rejection reason.',
        },
        types: {
          user: 'User',
          child: 'Child',
        },
        statuses: {
          pending: 'Pending',
          approved: 'Approved',
          rejected: 'Rejected',
        },
        errors: {
          load: 'Failed to load join requests.',
          review: 'Failed to review join request.',
        },
      },
      members: {
        title: 'Organization Users',
        subtitle: 'Staff and admin users in the organization.',
        searchPlaceholder: 'Search: full name, email, position, ID',
        filterStatus: 'Filter by status',
        empty: 'No organization users found.',
        position: 'Position',
        status: 'Status',
        headers: {
          id: 'ID',
          user: 'User',
          position: 'Position',
          status: 'Status',
          joinedAt: 'Joined at',
        },
        card: {
          id: 'ID: {value}',
          position: 'Position: {value}',
          status: 'Status: {value}',
          joinedAt: 'Joined at: {value}',
        },
        statuses: {
          active: 'Active',
          invited: 'Invited',
          blocked: 'Blocked',
        },
        errors: {
          load: 'Failed to load organization users.',
        },
      },
      clients: {
        title: 'Organization Clients',
        subtitle: 'Parents and children linked to the organization.',
        searchPlaceholder: 'Search: full name, email, ID',
        filterStatus: 'Filter by status',
        filterType: 'Filter by type',
        empty: 'No organization clients found.',
        subjectType: 'Subject type',
        subject: 'Subject',
        status: 'Status',
        headers: {
          id: 'ID',
          type: 'Type',
          subject: 'Subject',
          status: 'Status',
          joinedAt: 'Joined at',
        },
        card: {
          id: 'ID: {value}',
          type: 'Type: {value}',
          status: 'Status: {value}',
          joinedAt: 'Joined at: {value}',
        },
        statuses: {
          active: 'Active',
          left: 'Left',
          blocked: 'Blocked',
        },
        errors: {
          load: 'Failed to load organization clients.',
        },
      },
      errors: {
        invalidId: 'Invalid organization identifier.',
        load: 'Failed to load organization.',
      },
    },
    edit: {
      title: 'Edit organization',
      subtitle: 'Organization edit page.',
      saving: 'Saving...',
      errors: {
        invalidId: 'Invalid organization identifier.',
        load: 'Failed to load organization.',
        update: 'Failed to update organization.',
      },
    },
    fields: {
      name: 'Name',
      description: 'Description',
      country: 'Country',
      region: 'Region',
      address: 'Address',
      city: 'City',
      district: 'District',
      lat: 'Latitude',
      lng: 'Longitude',
      coordinates: 'Coordinates',
      metroStation: 'Metro station',
      travelMode: 'Travel mode',
      durationMinutes: 'Travel time (min)',
      walkDurationMinutes: 'Walk, min',
      driveDurationMinutes: 'Drive, min',
      phone: 'Phone',
      email: 'Email',
      status: 'Status',
      sourceType: 'Source',
      ownershipStatus: 'Ownership status',
      owner: 'Owner',
      ownerUserId: 'Owner ID',
      claimedAt: 'Claimed at',
      createdAt: 'Created at',
    },
    locations: {
      title: 'Locations',
      subtitle: 'An organization can have multiple addresses and multiple metro stations for each.',
      empty: 'No addresses added yet.',
      addLocation: 'Add address',
      removeLocation: 'Remove address',
      locationTitle: 'Address #{index}',
      countryPlaceholder: 'Select country',
      regionPlaceholder: 'Select region',
      cityPlaceholder: 'Select city',
      districtPlaceholder: 'Select district',
      metroTitle: 'Nearby metro',
      addMetro: 'Add metro',
      removeMetro: 'Remove metro',
      metroItemTitle: 'Metro #{index}',
      metroPlaceholder: 'Select station',
      metroEmpty: 'No metro stations added.',
      metroRequiresCity: 'Select a city first to load metro stations.',
      metroSummary: '#{index}: {station}, {mode}, {minutes} min.',
    },
    ownerPlaceholder: 'Search user: full name or ID',
    travelMode: {
      walk: 'Walk',
      drive: 'Drive',
    },
    status: {
      draft: 'Draft',
      active: 'Active',
      suspended: 'Suspended',
      archived: 'Archived',
    },
    source: {
      manual: 'Manual',
      import: 'Import',
      parsed: 'Parsed',
      selfRegistered: 'Self registered',
    },
    ownership: {
      unclaimed: 'Unclaimed',
      pendingClaim: 'Pending claim',
      claimed: 'Claimed',
    },
    confirmDelete: 'Delete organization {name}?',
    errors: {
      loadList: 'Failed to load organizations list.',
      delete: 'Failed to delete organization.',
    },
  },
  roles: {
    index: {
      title: 'Roles',
      subtitle: 'Search, sorting, limit and server pagination.',
      createLabel: 'New role',
      searchPlaceholder: 'Search: code or label',
      empty: 'No roles found.',
      headers: {
        code: 'Code',
        label: 'Label',
        type: 'Type',
        actions: 'Actions',
      },
      sort: {
        code: 'Code',
        label: 'Label',
        type: 'Type',
      },
      type: {
        system: 'System',
        custom: 'Custom',
      },
      cardLabelFallback: 'No label',
    },
    new: {
      title: 'New role',
      subtitle: 'Create role in `/api/admin/roles`.',
      fields: {
        code: 'Code',
        label: 'Label',
      },
      saving: 'Saving...',
      errors: {
        create: 'Failed to create role.',
      },
    },
    show: {
      title: 'Role',
      subtitle: 'Role show page.',
      labels: {
        code: 'Code',
        label: 'Label',
        type: 'Type',
      },
      errors: {
        invalidId: 'Invalid role identifier.',
        load: 'Failed to load role.',
      },
    },
    edit: {
      title: 'Edit role',
      subtitle: 'Role edit page.',
      fields: {
        code: 'Code',
        label: 'Label',
      },
      systemLocked: 'System roles cannot be edited.',
      saving: 'Saving...',
      errors: {
        invalidId: 'Invalid role identifier.',
        load: 'Failed to load role.',
        update: 'Failed to update role.',
      },
    },
    errors: {
      loadList: 'Failed to load roles.',
      delete: 'Failed to delete role.',
    },
    confirmDelete: 'Delete role {code}?',
  },
  geo: {
    title: 'Geography',
    countries: {
      index: {
        title: 'Countries',
        subtitle: 'Search, sorting, limit and server pagination.',
        createLabel: 'New country',
        searchPlaceholder: 'Search: name, ISO code',
        empty: 'No countries found.',
        headers: {
          name: 'Name',
          isoCode: 'ISO code',
          actions: 'Actions',
        },
        card: {
          isoCode: 'ISO code: {value}',
        },
        sort: {
          name: 'Name',
          isoCode: 'ISO code',
        },
      },
      new: {
        title: 'New country',
        subtitle: 'Create country in /api/admin/geo/countries',
        saving: 'Saving...',
        errors: {
          create: 'Failed to create country.',
        },
      },
      show: {
        title: 'Country',
        subtitle: 'Country details',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load country.',
        },
      },
      edit: {
        title: 'Edit country',
        subtitle: 'Update /api/admin/geo/countries/:id',
        saving: 'Saving...',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load country.',
          update: 'Failed to update country.',
        },
      },
      fields: {
        name: 'Name',
        isoCode: 'ISO code',
      },
      confirmDelete: 'Delete country {name}?',
      errors: {
        loadList: 'Failed to load countries list.',
        delete: 'Failed to delete country.',
      },
    },
    regions: {
      index: {
        title: 'Regions',
        subtitle: 'Search, sorting, limit and server pagination.',
        createLabel: 'New region',
        searchPlaceholder: 'Search: name, country',
        empty: 'No regions found.',
        headers: {
          name: 'Name',
          countryId: 'Country',
          actions: 'Actions',
        },
        card: {
          countryId: 'Country: {value}',
        },
        sort: {
          name: 'Name',
          countryId: 'Country',
        },
      },
      new: {
        title: 'New region',
        subtitle: 'Create region in /api/admin/geo/regions',
        saving: 'Saving...',
        errors: {
          create: 'Failed to create region.',
        },
      },
      show: {
        title: 'Region',
        subtitle: 'Region details',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load region.',
        },
      },
      edit: {
        title: 'Edit region',
        subtitle: 'Update /api/admin/geo/regions/:id',
        saving: 'Saving...',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load region.',
          update: 'Failed to update region.',
        },
      },
      fields: {
        name: 'Name',
        countryId: 'Country ID',
        country: 'Country',
      },
      confirmDelete: 'Delete region {name}?',
      errors: {
        loadList: 'Failed to load regions list.',
        delete: 'Failed to delete region.',
      },
    },
    cities: {
      index: {
        title: 'Cities',
        subtitle: 'Search, sorting, limit and server pagination.',
        createLabel: 'New city',
        searchPlaceholder: 'Search: name, country, region',
        empty: 'No cities found.',
        headers: {
          name: 'Name',
          countryId: 'Country',
          regionId: 'Region',
          actions: 'Actions',
        },
        card: {
          countryId: 'Country: {value}',
          regionId: 'Region: {value}',
        },
        sort: {
          name: 'Name',
          countryId: 'Country',
          regionId: 'Region',
        },
      },
      new: {
        title: 'New city',
        subtitle: 'Create city in /api/admin/geo/cities',
        saving: 'Saving...',
        errors: {
          create: 'Failed to create city.',
        },
      },
      show: {
        title: 'City',
        subtitle: 'City details',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load city.',
        },
      },
      edit: {
        title: 'Edit city',
        subtitle: 'Update /api/admin/geo/cities/:id',
        saving: 'Saving...',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load city.',
          update: 'Failed to update city.',
        },
      },
      fields: {
        name: 'Name',
        countryId: 'Country ID',
        regionId: 'Region ID',
        country: 'Country',
        region: 'Region',
      },
      confirmDelete: 'Delete city {name}?',
      errors: {
        loadList: 'Failed to load cities list.',
        delete: 'Failed to delete city.',
      },
    },
    districts: {
      index: {
        title: 'Districts',
        subtitle: 'Search, sorting, limit and server pagination.',
        createLabel: 'New district',
        searchPlaceholder: 'Search: name, city',
        empty: 'No districts found.',
        headers: {
          name: 'Name',
          cityId: 'City',
          actions: 'Actions',
        },
        card: {
          city: 'City: {value}',
        },
        sort: {
          name: 'Name',
          cityId: 'City',
        },
      },
      new: {
        title: 'New district',
        subtitle: 'Create district in /api/admin/geo/districts',
        saving: 'Saving...',
        errors: {
          create: 'Failed to create district.',
        },
      },
      show: {
        title: 'District',
        subtitle: 'District details',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load district.',
        },
      },
      edit: {
        title: 'Edit district',
        subtitle: 'Update /api/admin/geo/districts/:id',
        saving: 'Saving...',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load district.',
          update: 'Failed to update district.',
        },
      },
      fields: {
        name: 'Name',
        cityId: 'City ID',
        city: 'City',
      },
      confirmDelete: 'Delete district {name}?',
      errors: {
        loadList: 'Failed to load districts list.',
        delete: 'Failed to delete district.',
      },
    },
  },
  metro: {
    title: 'Metro',
    lines: {
      index: {
        title: 'Metro lines',
        subtitle: 'Search, sorting, limit and server pagination.',
        createLabel: 'New line',
        searchPlaceholder: 'Search: name, line ID, city',
        empty: 'No metro lines found.',
        headers: {
          name: 'Name',
          lineId: 'Line ID',
          color: 'Color',
          cityId: 'City',
          actions: 'Actions',
        },
        card: {
          lineId: 'Line ID: {value}',
          color: 'Color:',
          cityId: 'City: {value}',
        },
        sort: {
          name: 'Name',
          lineId: 'Line ID',
          cityId: 'City',
        },
      },
      new: {
        title: 'New metro line',
        subtitle: 'Create line in /api/admin/metro-lines',
        saving: 'Saving...',
        errors: {
          create: 'Failed to create metro line.',
        },
      },
      show: {
        title: 'Metro line',
        subtitle: 'Metro line details',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load metro line.',
        },
      },
      edit: {
        title: 'Edit metro line',
        subtitle: 'Update /api/admin/metro-lines/:id',
        saving: 'Saving...',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load metro line.',
          update: 'Failed to update metro line.',
        },
      },
      fields: {
        name: 'Name',
        externalId: 'External ID',
        lineId: 'Line ID',
        color: 'Color',
        cityId: 'City ID',
        city: 'City',
        source: 'Source',
      },
      confirmDelete: 'Delete metro line {name}?',
      errors: {
        loadList: 'Failed to load metro lines list.',
        delete: 'Failed to delete metro line.',
      },
    },
    stations: {
      index: {
        title: 'Metro stations',
        subtitle: 'Search, sorting, limit and server pagination.',
        createLabel: 'New station',
        searchPlaceholder: 'Search: name, line ID, metro line, city ID',
        empty: 'No metro stations found.',
        headers: {
          name: 'Name',
          lineId: 'Line ID',
          metroLine: 'Metro line',
          city: 'City',
          actions: 'Actions',
        },
        card: {
          lineId: 'Line ID: {value}',
          metroLine: 'Metro line: {value}',
          city: 'City: {value}',
        },
        sort: {
          name: 'Name',
          lineId: 'Line ID',
          metroLine: 'Metro line',
          city: 'City',
        },
      },
      new: {
        title: 'New metro station',
        subtitle: 'Create station in /api/admin/metro-stations',
        saving: 'Saving...',
        metroLinePlaceholder: 'Select metro line',
        cityPlaceholder: 'Select city',
        errors: {
          create: 'Failed to create metro station.',
        },
      },
      show: {
        title: 'Metro station',
        subtitle: 'Metro station details',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load metro station.',
        },
      },
      edit: {
        title: 'Edit metro station',
        subtitle: 'Update /api/admin/metro-stations/:id',
        saving: 'Saving...',
        metroLinePlaceholder: 'Select metro line',
        cityPlaceholder: 'Select city',
        errors: {
          invalidId: 'Invalid ID.',
          load: 'Failed to load metro station.',
          update: 'Failed to update metro station.',
        },
      },
      fields: {
        name: 'Name',
        externalId: 'External ID',
        lineId: 'Line ID',
        geo: 'Geo',
        geoLat: 'Geo lat',
        geoLon: 'Geo lon',
        isClosed: 'Station closed',
        metroLine: 'Metro line',
        metroLineId: 'Metro Line ID',
        cityId: 'City ID',
        source: 'Source',
      },
      status: {
        closedYes: 'Yes',
        closedNo: 'No',
      },
      confirmDelete: 'Delete metro station {name}?',
      errors: {
        loadList: 'Failed to load metro stations list.',
        delete: 'Failed to delete metro station.',
      },
    },
  },
  actionLogs: {
    title: 'Change log',
    subtitle: 'Model change history (create, update, delete).',
    empty: 'No records yet.',
    headers: {
      createdAt: 'Date',
      event: 'Event',
      model: 'Entity',
      user: 'Actor',
      changedFields: 'Changed fields',
      ip: 'IP',
    },
    events: {
      create: 'Create',
      update: 'Update',
      delete: 'Delete',
    },
    models: {
      user: 'User',
      role: 'Role',
      child: 'Child',
      organization: 'Organization',
      metroLine: 'Metro line',
      metroStation: 'Metro station',
      geoCountry: 'Country',
      geoRegion: 'Region',
      geoCity: 'City',
      geoDistrict: 'District',
    },
    filters: {
      event: 'Event',
      anyEvent: 'Any event',
      model: 'Entity',
      anyModel: 'Any entity',
      user: 'User',
      userPlaceholder: 'ID / email / full name',
      dateRange: 'Date range',
      dateFrom: 'Date from',
      dateTo: 'Date to',
      searchPlaceholder: 'Search by entity/event/id',
      eventAll: 'All',
      eventCreate: 'Create',
      eventUpdate: 'Update',
      eventDelete: 'Delete',
      modelAll: 'All',
      modelUser: 'User',
      modelRole: 'Role',
      modelChild: 'Child',
      modelOrganization: 'Organization',
      modelMetroLine: 'Metro line',
      modelMetroStation: 'Metro station',
      modelGeoCountry: 'Country',
      modelGeoRegion: 'Region',
      modelGeoCity: 'City',
      modelGeoDistrict: 'District',
    },
    errors: {
      load: 'Failed to load action logs.',
    },
  },
  errors: {
    users: {
      loadList: 'Failed to load users.',
      delete: 'Failed to delete user.',
    },
  },
} as const;
