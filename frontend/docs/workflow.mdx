...
- For public SSR changes:
  - run `make cache-reset-all` before final verification;
  - run SSR/SEO checklist from `ssr-and-seo-checklist.mdx`;
  - review performance budget for `/` and `/catalog/**` from `performance-budget.mdx`;
  - pass the final handoff gate from `public-qa-gate.mdx`;
  - if budget is exceeded, add explicit PR justification.
...
- `frontend/docs` обновлён при изменении архитектурных или контрактных договорённостей.
- Для изменений public SSR:
  - перед финальной проверкой выполнить `make cache-reset-all`;
  - пройти SSR/SEO чеклист из `ssr-and-seo-checklist.mdx`;
  - проверить performance budget для `/` и `/catalog/**` из `performance-budget.mdx`;
  - пройти финальный gate из `public-qa-gate.mdx`;
  - при выходе за budget добавить явное обоснование в PR.
